...
const FoodCard = ({ post }) => {
    ...
    return (
        <div>
            {post &&
                <div className='bg-gray w-[20rem] rounded-md h-[25rem]' onClick={handleFoodClick}>
                    ...
                    <div className='flex flex-col items-end justify-center p-5 gap-2'>
                        ...
                        <span className='flex flex-row-reverse justify-center items-center gap-3'><IoStar className='text-[#FFCD3C]' /> {post.owner.rating}</span>
                    </div>
                </div>
            }
        </div>
